module.exports.summarizeData = function summarizeData(initArr, sumArr){ // Function to summarize the collected data and remove any duplicates which may arise
  ...
  while(count <= initArr.length){ // A while loop will help to ensure that the resulting array has looped through all of the data present in the initial array of data, rather than ending prematurely
    for(var i = 0, j = 1; i < initArr.length, j < initArr.length; j++){
      ...
      if(j === i){} // Empty if to prevent matching and splicing on the same position in the array
      else if(min_time === 0){ // If min_time is equal to 0
        min_time = Arr1["elapsed_time"]; // Set min_time equal to the elapsed_time value of the static array pointer
      }
        ...
        if(max_time < Arr2["elapsed_time"]){ // If the value of max_time is less than that of the elapsed_time value of the moving array pointer
          max_time = Arr2["elapsed_time"]; // Set max_time to the elapsed_time value of the moving array pointer
        }
        if(min_time > Arr2["elapsed_time"]){ // If the value of min_time is greater than that of the elapsed_time value of the moving array pointer
          min_time = Arr2["elapsed_time"]; // Set min_time to the elapsed_time value of the moving array pointer
        }
        ...
        if(max_time < Arr1["elapsed_time"]){ // If the value of max_time is less than that of the elapsed_time value of the static array pointer
          max_time = Arr1["elapsed_time"]; // Set max_time to the elapsed_time value of the static array pointer
        }
        ...
        j = 0; // Set the moving array pointer to 0 to catch any potential matches before the static array pointer (note: without the empty if this would result in i and j being at the same position and splicing, thus resulting in a complete loss of data)
        total_time = 0;
        total_count = 0;
        average_time = 0;
        min_time = 0;
  ...
  while(count < sumArr.length){ // While loop to ensure that no duplicate data has managed to slip through the previous loop
