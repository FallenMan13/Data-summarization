...
module.exports.beginRequest = function beginRequest(JSONrequest){ // Function to allow the user to decide to use the default dates or enter their own
  ...
  defaults("Would you like to use the default dates: " + midNight + " to " + morning1 + "? ", function(answer){
    rlinterface.close();
    JSONrequest.query.bool.must[1].range["@timestamp"].gte = Date.parse(midNight); // Parse the first default date into epoch milliseconds and store in the JSON body
    JSONrequest.query.bool.must[1].range["@timestamp"].lte = Date.parse(morning1); // Parse the second default date into epoch milliseconds and store in the JSON body
    page.getPage(undefined, JSONrequest); // Send the altered JSON body to the "getPage" module
  })
}
var counter = 0;
function dateRange(JSONrequest){ // Function to allow the user to enter a date range to search with
    ...
    function analyzeAnswer(answer){
      rlinterface.question("Is this date correct? " + answer + " ", function(confirm){
        if(confirm.toLowerCase().startsWith("y")){ // If the response entered begins with a 'y' when converted to lowercase
          ...
          rlinterface.question(qstntxt[counter], analyzeAnswer); // Retry the question
        }
      })
    }
  ...
  dates(qstntxt[counter], function(answer){ // Function to parse the entered dates into the JSON body and send it to the "getPage" module
    if(counter === 0){ // If the first question has been asked
      JSONrequest.query.bool.must[1].range["@timestamp"].gte = Date.parse(answer); // Parse the entered date into epoch milliseconds and store in the JSON body
      ...
      rlinterface.close();
      JSONrequest.query.bool.must[1].range["@timestamp"].lte = Date.parse(answer); // Parse the entered date into epoch milliseconds and store in the JSON body
      page.getPage(undefined, JSONrequest); // Send the altered JSON body to the "getPage" module
    }
  })
}
